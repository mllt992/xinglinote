...
- 实例管理员在实例后台「量化管理」查看全站未删除笔记，按工作区、成员、笔记本、量化状态、笔记/工作区 AI 开关、模型配置、更新时间和排序组合筛选，列表服务端分页。可把一套共享 Embedding 渠道与模型应用到全部工作区并排队；关闭了“AI 可读”或工作区 AI 的内容也可以预先建向量，但搜索、问答和 MCP 仍遵守权限与开关。工作区「量化队列」只管理本区；普通工作区管理员不能进实例后台。
...
范围：当前笔记本 / 当前工作区。输入问题。上游使用 OpenAI 兼容 SSE，本站以 NDJSON 逐块转发；前端收到一块就追加一块，结束事件再补引用、版本校验与用量。答案按 Markdown 渲染（与正文同一套 `renderMarkdown`，侧栏用短文一档字号）。每条答案下挂引用：标题 + 片段，点击打开笔记定位。
