...
import { aiProvider as provider, chatAi as chat } from "../lib/ai.ts";
...
function keySuffix(value:string){try{return suffix(value);}catch{return "";}}
aiRoutes.get("/workspaces/:id/ai/provider",async c=>{const {u,role}=await member(c,c.req.param("id"));const rows=await db.select().from(aiProviders).where(eq(aiProviders.workspaceId,c.req.param("id"))).orderBy(desc(aiProviders.createdAt));return ok(c,{providers:rows.filter(p=>!p.ownerUserId||p.ownerUserId===u.id).map(p=>({id:p.id,kind:p.kind,baseUrl:p.baseUrl,chatModel:p.chatModel,embeddingModel:p.embeddingModel,keySuffix:keySuffix(p.apiKey),ownerUserId:p.ownerUserId,enabled:p.enabled,canManage:role==="owner"||role==="admin"||p.ownerUserId===u.id}))});});
aiRoutes.post("/workspaces/:id/ai/provider",async c=>{const {u,role}=await ctx(c,c.req.param("id"));const body=z.object({baseUrl:z.string().url(),chatModel:z.string().min(1),embeddingModel:z.string().optional(),apiKey:z.string().min(1),personal:z.boolean().default(false)}).parse(await c.req.json());if(!body.personal&&role!=="owner"&&role!=="admin")throw fail("FORBIDDEN","只有管理员能配置工作区 Key");const baseUrl=body.baseUrl.replace(/\/$/,"");await assertSafeOutboundUrl(baseUrl,"AI 提供商地址");const [p]=await db.insert(aiProviders).values({workspaceId:c.req.param("id"),ownerUserId:body.personal?u.id:null,baseUrl,chatModel:body.chatModel,embeddingModel:body.embeddingModel,apiKey:seal(body.apiKey)}).returning();return ok(c,{id:p.id,keySuffix:suffix(p.apiKey)},201);});
