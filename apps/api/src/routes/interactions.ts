...
async function publicChannel(c:Parameters<typeof getCookie>[0],noteId:string,shareToken?: string, siteNotebookId?: string) {
  if (shareToken) { const [s] = await db.select().from(shareLinks).where(eq(shareLinks.token, shareToken)); if (!s || s.status !== "active" || (s.targetType!=="note"&&s.targetType!=="heading")||s.targetId!==noteId||(s.expiresAt && s.expiresAt.getTime() <= Date.now())) throw fail("NOT_FOUND", "分享不存在");if(s.passwordHash&&getCookie(c,shareCookie(s.token))!==s.id)throw fail("FORBIDDEN","请先解锁分享"); return { shareId: s.id, siteNotebookId: null, comments: s.commentsEnabled, corrections: s.correctionsEnabled }; }
...
async function notifyEditors(noteId: string, title: string, body: string) { const [note] = await db.select().from(notes).where(eq(notes.id, noteId)); if (!note) return; const members = await db.select().from(workspaceMembers).where(eq(workspaceMembers.workspaceId, note.workspaceId)); const ids = new Set([note.createdBy, ...members.filter(m => m.role !== "viewer").map(m => m.userId)]); if (ids.size) await db.insert(notifications).values([...ids].map(userId => ({ userId, type: "interaction_pending", title, body, href: `/w/${note.workspaceId}/n/${note.id}` }))); }

interactionRoutes.get("/public/notes/:id/comments", async c => { const noteId = c.req.param("id"); const channel=await publicChannel(c,noteId,c.req.query("shareToken"), c.req.query("siteNotebookId")); const rows = await db.select().from(comments).where(and(eq(comments.targetType, "note"), eq(comments.targetId, noteId),channel.shareId?eq(comments.shareId,channel.shareId):eq(comments.siteNotebookId,channel.siteNotebookId!), eq(comments.status, "visible"))).orderBy(desc(comments.createdAt)); const authors = await db.select().from(users); const viewer = await currentUser(c); return ok(c, { comments: rows.map(r => ({ id: r.id, body: r.body, parentId: r.parentId, author: r.authorUserId ? authors.find(u => u.id === r.authorUserId)?.displayName ?? "已注销用户" : r.guestName, createdAt: r.createdAt, editedAt: r.editedAt, mine: !!viewer && r.authorUserId === viewer.id, editableUntil: new Date(new Date(r.createdAt).getTime() + 300000) })) }); });
interactionRoutes.post("/public/notes/:id/comments", async c => { const noteId = c.req.param("id"); const channel = await publicChannel(c,noteId,c.req.query("shareToken"), c.req.query("siteNotebookId")); if (!channel.comments) throw fail("FORBIDDEN", "评论已关闭"); const [note] = await db.select().from(notes).where(eq(notes.id, noteId)); if (!note || note.trashedAt) throw fail("NOT_FOUND", "笔记不存在"); const user = await currentUser(c); const body = z.object({ body: z.string().min(1).max(2000), parentId: z.string().uuid().nullish(), guestName: z.string().min(1).max(40).optional(), guestEmail: z.string().email().optional(), challengeToken: z.string().optional(), challengeAnswer: z.string().optional() }).parse(await c.req.json());
  ...
  if (user) limit(`comment:user:${user.id}:${note.id}`, 10, 60000); else limit(`comment:ip:${ip}`, 5, 600000);
...
interactionRoutes.get("/public/captcha", async c => { limit(`captcha:${c.req.header("x-forwarded-for") ?? "local"}`, 30, 60000); return ok(c, issueChallenge()); });
...
interactionRoutes.get("/notifications", async c => { const u=await currentUser(c);if(!u)throw fail("UNAUTHENTICATED","未登录"); const rows=await db.select().from(notifications).where(eq(notifications.userId,u.id)).orderBy(desc(notifications.createdAt));return ok(c,{notifications:rows.slice(0,50)}); });
