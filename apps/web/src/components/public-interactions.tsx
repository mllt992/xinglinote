import{useEffect,useState}from'react';import{MessageSquare,PencilLine,Reply,Send}from'lucide-react';import{api}from'../api';import{Button}from'./ui/button';import{Input}from'./ui/input';import{Textarea}from'./ui/textarea';import{Badge}from'./ui/badge';
...
export function PublicInteractions({noteId,shareToken,siteNotebookId,commentsEnabled,correctionsEnabled,bodyMd}:{noteId:string;shareToken?:string;siteNotebookId?:string;commentsEnabled:boolean;correctionsEnabled:boolean;bodyMd:string}){
  ...
  const[comments,setComments]=useState<Comment[]>([]);const[body,setBody]=useState("");const[name,setName]=useState("");const[msg,setMsg]=useState("");
  ...
  async function send(e:React.FormEvent){
    ...
    try{
      ...
      setBody("");setReplyTo(null);setMsg(d.status==="pending"?"评论已提交，等待作者审核。":"评论已发布。");void newChallenge();void load();
    }catch(x){setMsg((x as Error).message);void newChallenge();}
  }
  ...
  return <section className="mx-auto mt-16 max-w-3xl border-t pt-10">
    ...
    {correctionsEnabled&&fixOpen&&<form className="mt-5 space-y-3 rounded-xl border bg-muted/30 p-4" onSubmit={async e=>{e.preventDefault();setMsg("");
      try{await api(`/api/v1/public/notes/${noteId}/corrections?${source}`,{method:"POST",body:JSON.stringify({...fix,guestName:name||undefined})});setFix({originalExcerpt:"",suggested:"",comment:""});setFixOpen(false);setMsg("纠错已提交，等待作者审核。")}catch(x){setMsg((x as Error).message)}}}>
    ...
    <div className="mt-5 space-y-4">{roots.length===0?<p className="text-sm text-muted-foreground">还没有公开评论。</p>:roots.map(c=><div key={c.id} className="rounded-xl bg-muted/50 p-4">
      <div className="flex items-center gap-2 text-xs text-muted-foreground"><b className="text-foreground">{c.author}</b><span>{new Date(c.createdAt).toLocaleString()}</span>{c.editedAt&&<Badge>已编辑</Badge>}
        <span className="ml-auto flex gap-1">{commentsEnabled&&<Button variant="ghost" size="sm" onClick={()=>{setReplyTo(c);setEditing(null)}}><Reply/>回复</Button>}
          {editable(c)&&<Button variant="ghost" size="sm" onClick={()=>{setEditing(c);setDraft(c.body);setReplyTo(null)}}><PencilLine/>编辑</Button>}</span></div>
      {editing?.id===c.id
        ? <form className="mt-2 space-y-2" onSubmit={async e=>{e.preventDefault();setMsg("");try{await api(`/api/v1/public/comments/${c.id}`,{method:"PATCH",body:JSON.stringify({body:draft})});setEditing(null);void load()}catch(x){setMsg((x as Error).message)}}}>
            <Textarea value={draft} onChange={e=>setDraft(e.target.value)} className="min-h-16"/><div className="flex gap-2"><Button size="sm">保存</Button><Button size="sm" variant="ghost" type="button" onClick={()=>setEditing(null)}>取消</Button><span className="self-center text-[11px] text-muted-foreground">发布后 5 分钟内可改</span></div></form>
    ...
    {commentsEnabled?<form className="mt-6 space-y-3" onSubmit={send}>
      ...
      <div className="flex items-center gap-3"><Button disabled={!body.trim()}><Send/>发表评论</Button>{msg&&<span className="text-xs text-muted-foreground">{msg}</span>}</div>
    </form>:<p className="mt-6 text-sm text-muted-foreground">这条链接关闭了评论。{msg&&` ${msg}`}</p>}
  </section>;
}
