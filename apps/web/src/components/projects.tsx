...
export function ProjectsPage() {
  ...
  useEffect(() => { api<{ workspaces: Array<{ id: string; name: string }> }>("/api/v1/workspaces").then(d => setSpaces(d.workspaces)).catch(() => {}); }, []);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const d = await api<{ projects: Project[]; canEdit: boolean; rhythm: { completedThisWeek: number; minutesThisWeek: number } }>(
        `/api/v1/workspaces/${wsId}/projects${showArchived ? "?archived=1" : ""}`,
      );
    ...
    <header className="sticky top-0 z-30 shrink-0 border-b border-border bg-background">
      <div className="mx-auto flex h-14 w-full max-w-[1180px] items-center gap-2 px-4 sm:px-6">
        <button className="-mx-1 flex items-center gap-2 rounded-lg px-1.5 py-1.5 hover:bg-muted" onClick={() => nav(`/w/${wsId}`)}>
          ...
          <span className="max-w-40 truncate text-sm font-semibold">{spaces.find(s => s.id === wsId)?.name ?? "项目"}</span>
        </button>
      ...
      <div className="mx-auto w-full max-w-[1180px] px-4 py-8 sm:px-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground">交付面，不是第二个待办。日历管日期，这里管交不交得出去。</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {canEdit && <Button size="sm" onClick={() => setCreating(true)}><Plus />建项目</Button>}
          </div>
        </div>
        {!showArchived && <p className="mt-4 text-sm text-muted-foreground">本周完成 {rhythm.completedThisWeek} 件 · 记下 {rhythm.minutesThisWeek} 分钟</p>}
        ...
        {loading ? <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{Array.from({ length: 3 }, (_, i) => <div key={i} className="h-36 animate-pulse rounded-xl border bg-muted/50" />)}</div>
          : !projects.length ? <div className="mt-16 text-center">
            <p className="text-sm font-medium">{showArchived ? "还没有归档的项目" : "项目是交付面，不是第二个待办"}</p>
            {!showArchived && canEdit && <Button className="mt-4" onClick={() => setCreating(true)}><Plus />建第一个项目</Button>}
          </div>
          : <div className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            {projects.map(p => <button key={p.id} onClick={() => nav(`/w/${wsId}/projects/${p.id}`)} className="rounded-xl border border-border bg-background p-4 text-left transition hover:border-foreground/20 hover:shadow-sm">
              <div className="flex items-start gap-2">
                <span className={cn("mt-1 size-2.5 shrink-0 rounded-full", COLOR_DOT[p.color])} />
                <div className="min-w-0 flex-1">
                  <p className="truncate font-medium">{p.title}</p>
                  <p className="mt-1 line-clamp-2 text-xs text-muted-foreground">{p.description || "还没写说明"}</p>
                </div>
                <Badge>{STATUS_LABEL[p.status]}</Badge>
              </div>
              <div className="mt-4 flex items-center justify-between text-xs">
                <span className={cn("font-medium tabular-nums", healthTone(p.health.band))}>{p.health.score} {HEALTH_BAND_LABEL[p.health.band]}</span>
                <span className="text-muted-foreground">{p.doneCount}/{p.taskCount} · {fmtSec(p.actualSeconds)}</span>
              </div>
              {p.dueAt && <p className="mt-2 text-[11px] text-muted-foreground">截止 {isoDate(p.dueAt)}</p>}
            </button>)}
          </div>}
      </div>
    </main>
    <ProjectDialog open={creating} onOpenChange={setCreating} onSubmit={async body => {
      const created = await api<Project>(`/api/v1/workspaces/${wsId}/projects`, { method: "POST", body: JSON.stringify(body) });
      toast.success("已建项目");
      nav(`/w/${wsId}/projects/${created.id}`);
    }} />
  </div>;
}
...
export function ProjectPage() {
  ...
  return <div className="flex h-full min-h-0 flex-col bg-background">
    <header className="flex h-14 shrink-0 items-center gap-2 border-b border-border px-3 sm:px-4">
      <Button variant="ghost" size="sm" onClick={() => nav(`/w/${wsId}/projects`)}><ChevronRight className="rotate-180" />项目</Button>
...
function ProjectDialog({ open, onOpenChange, onSubmit, project, canChangeVisibility = true }: {
  open: boolean; onOpenChange: (v: boolean) => void;
  onSubmit: (body: Record<string, unknown>) => Promise<void>;
  project?: Project; canChangeVisibility?: boolean;
}) {
  ...
  async function submit(e: FormEvent) {
    ...
    setBusy(true); setErr("");
    try {
      await onSubmit({ title: title.trim(), description, status, color, startAt: toIso(startAt), dueAt: toIso(dueAt), ...(canChangeVisibility ? { visibility } : {}) });
    } catch (x) { setErr((x as Error).message); }
    finally { setBusy(false); }
  }
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent>
    <DialogHeader><DialogTitle>{project ? "编辑项目" : "建项目"}</DialogTitle><DialogDescription>项目回答「什么时候交、卡在哪、花了多久」。不要做成第二个待办列表。</DialogDescription></DialogHeader>
    <form className="space-y-3" onSubmit={submit}>
      <Input autoFocus value={title} onChange={e => setTitle(e.target.value)} placeholder="项目名" />
      ...
      <div className="flex justify-end gap-2"><Button type="button" variant="ghost" onClick={() => onOpenChange(false)}>取消</Button><Button disabled={busy || !title.trim()}>{busy ? "保存中…" : "保存"}</Button></div>
    </form>
